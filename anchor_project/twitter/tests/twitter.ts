...
describe("twitter", () => {
  ...
  describe("Initialize Tweet", async () => {
    it("Initialize Tweet!", async () => {
      ...
      let twitterData = await program.account.tweet.fetch(tweet_pkey);
      const utf8ByteArray_topic = stringToUtf8ByteArray(topic_bob1);
      const paddedByteArray_topic = padByteArrayWithZeroes(utf8ByteArray_topic, 32);

      const utf8ByteArray_content = stringToUtf8ByteArray(content_bob1);
      const paddedByteArray_content = padByteArrayWithZeroes(utf8ByteArray_content, 500);

      assert.strictEqual(twitterData.topic.toString(), paddedByteArray_topic.toString());
      assert.strictEqual(twitterData.content.toString(), paddedByteArray_content.toString());
      assert.strictEqual(twitterData.bump.toString(), tweet_bump.toString())
      assert.strictEqual(twitterData.tweetAuthor.toString(), bob.publicKey.toString())
      assert.strictEqual(twitterData.likes.toString(), new anchor.BN(0).toString())
      assert.strictEqual(twitterData.dislikes.toString(), new anchor.BN(0).toString())
    });
    ...
    it("Bob can Initialize second Tweet!", async () => {
      ...
      let twitterData = await program.account.tweet.fetch(tweet_pkey);
      const utf8ByteArray_topic = stringToUtf8ByteArray(topic_bob4);
      const paddedByteArray_topic = padByteArrayWithZeroes(utf8ByteArray_topic, 32);

      const utf8ByteArray_content = stringToUtf8ByteArray(content_bob4);
      const paddedByteArray_content = padByteArrayWithZeroes(utf8ByteArray_content, 500);
  ...
  describe("Alice wants to react on tweet!", async () => {
    it("Alice can like Bob`s Tweet 1!", async () => {
      ...
      let twitterData = await program.account.tweet.fetch(tweet_pkey);
      assert.strictEqual(twitterData.likes.toString(), new anchor.BN(1).toString())
      assert.strictEqual(twitterData.dislikes.toString(), new anchor.BN(0).toString())
    ...
    it("Alice dislikes Bob`s second Tweet!", async () => {
      ...
      let twitterData = await program.account.tweet.fetch(tweet_pkey);
      assert.strictEqual(twitterData.likes.toString(), new anchor.BN(0).toString())
      assert.strictEqual(twitterData.dislikes.toString(), new anchor.BN(1).toString())
  ...
  describe("Alice decided that maybe small comment about Bob`s tweet would be great!", async () => {
    ...
    it("But if the comment is not that long she can tweet on Bob`s second tweet!", async () => {
      ...
      const utf8ByteArray_comment = stringToUtf8ByteArray(comment_alice2);
      const paddedByteArray_comment = padByteArrayWithZeroes(utf8ByteArray_comment, 500);

      assert.strictEqual(commentData.commentAuthor.toString(), alice.publicKey.toString());
      assert.strictEqual(commentData.content.toString(), paddedByteArray_comment.toString());
      assert.strictEqual(commentData.contentLength.toString(), utf8ByteArray_comment.length.toString());
      assert.strictEqual(commentData.parentTweet.toString(), tweet_pkey.toString());
      assert.strictEqual(commentData.bump.toString(), comment_bump.toString());
