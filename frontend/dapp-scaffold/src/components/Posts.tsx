      ...
      <div className="flex flex-row justify-center">
        <div className="relative group items-center">
          <>
            {posts.map((post) => {
              return (
                <div key={post.publicKey}>
                  Author: {post.account.tweetAuthor.toString()} <br />
                  Topic: {String.fromCharCode(...post.account.topic)} <br />
                  Content: {String.fromCharCode(...post.account.content)} <br />
                  Likes: {post.account.likes.toString()} Dislikes:{" "}
                  {post.account.dislikes.toString()}
                  <br />
                  ----------comments----------
                  <div>
                    {comments.filter((comment) => { return comment.account.parentTweet.toString() === post.publicKey.toString()}).map((comment) => {
                      return (
                        ...
                        Author: {comment.account.commentAuthor.toString()} <br />    
                        Content: {String.fromCharCode(...comment.account.content)} <br />
                        ---------
                        </div>
                      );
                    })}
                  ...
                  <button className="btn btn-primary" onClick={() => like(post)} disabled={!wallet.publicKey}>
                    <svg
                  ...
                  <button className="btn btn-primary" onClick={() => dislike(post)} disabled={!wallet.publicKey}>
                    <svg
                      viewBox="0 0 1024 1024"
                      fill="currentColor"
                      height="1em"
